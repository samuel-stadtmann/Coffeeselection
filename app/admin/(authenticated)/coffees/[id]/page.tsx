...
async function loadCatalogs() {
  ...
  const [
    ...
    sb.from("brewing_methods_catalog").select("id, name_de, category").eq("active", true).order("sort_order"),
    sb.from("flavor_notes_catalog").select("id, name_de, family").eq("active", true).order("sort_order"),
  ]);
...
export default async function AdminEditCoffeePage({
  params,
}: {
  ...
  const initial: CoffeeFormState = {
    ...
    roast_profile_touched: coffee.roast_profile != null,
    ...
    <>
      <div className="mb-8">
        ...
        <p className="text-on-surface-variant text-sm max-w-2xl">
          Vollzugriff als Admin. Statuswechsel auf <strong>active</strong> ist
          erlaubt (im Gegensatz zum Röster-Edit).
        </p>
      </div>
      ...
      <CoffeeForm
        ...
        submitEndpoint={`/api/admin/coffees/${id}`}
        submitMethod="PATCH"
