...
export default async function AdminCustomerDetailPage({
  ...
  const [
    ...
    svc
      ...
      .select(
        "id, type, is_default, recipient_name, street, street_additional, postal_code, city, region, country"
      )
      .eq("customer_id", id)
      ...
      <section className="bg-white p-6 md:p-8 shadow-sm">
        ...
        {addressList.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {addressList.map((a) => (
              <div key={a.id} className="bg-surface-container-low p-4 text-sm">
                ...
                <p>{a.street}</p>
                {a.street_additional && <p>{a.street_additional}</p>}
                <p>
                  {a.postal_code} {a.city}
                  {a.region && `, ${a.region}`}
                </p>
                <p className="text-on-surface-variant">{a.country}</p>
              </div>
            ))}
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead className="font-headline text-[10px] uppercase tracking-widest text-on-surface-variant font-bold">
                <tr>
                  ...
                  <th className="text-left py-2">Intervall</th>
                  <th className="text-right py-2">Preis/Lieferung</th>
              ...
              <tbody className="divide-y divide-surface-container">
                {subList.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td className="py-2">
                      {s.discovery_mode ? "Discovery" : "Fix"}
                    </td>
                    <td className="py-2">{s.interval_weeks} Wo</td>
                    <td className="py-2 text-right">
                      {fmtChf(Number(s.price_chf_per_delivery))}
                    </td>
