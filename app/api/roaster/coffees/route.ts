...
// POST /api/roaster/coffees
//   Body: CoffeeFormState (siehe lib/coffee/form-helpers.ts)
//   Permission:
//     1. User muss in roaster_users sein.
//     2. submitted roaster_id muss zu einer der memberships gehoeren —
//        sonst koennte ein Roester-User einen Coffee fuer eine fremde
//        Roesterei einreichen.
//   Aktion:
//     1. Insert in coffees mit status='draft'.
//     2. Insert in coffee_allergens + coffee_certifications.
//     3. Database Webhook generiert flavor_embedding automatisch.
...
export async function POST(req: NextRequest) {
  ...
  // Permission-Check: der eingereichte roaster_id MUSS zu einer Membership
  // des Users passen — sonst koennte ein User Coffees fuer fremde Roester
  // einreichen.
  const allowedRoasters = u.memberships.map((m) => m.roaster_id);
  if (!allowedRoasters.includes(parsed.roaster_id)) {
    return NextResponse.json(
      { error: "forbidden", details: "roaster_id ist nicht in deinen Memberships" },
      { status: 403 }
    );
  }
  ...
  if (allergen_slugs.length > 0) {
    ...
    if (allErr) console.error("[roaster/coffees] allergen link failed", allErr);
  }
