-- ============================================================================
-- M5d Followup — Fix latente Spalten-Ambiguitaet in rank_coffees_for_customer
--
-- Die Function deklariert RETURNS TABLE(rank, coffee_id, coffee_name, ...).
-- Diese OUT-Parameter werden in PL/pgSQL als Variablen sichtbar und kollidieren
-- mit gleichnamigen Spalten in tmp_scored / tmp_pool / tmp_results. Im
-- Discovery-Pfad (MMR) faellt das auf:
--
--   ERROR: column reference "coffee_id" is ambiguous (line ~140)
--
-- Fix: Funktion mit #variable_conflict use_column rekonstruieren — alle
-- unqualifizierten Namen-Konflikte werden zur Spalte aufgeloest. Logik
...
begin
  ...
  -- 4) Soft-Scoring + Vector + vorlaeufiger Final-Score
  ...
  -- 5) MMR (Discovery + Embeddings) | Top-N (Fix oder Cold-Start)
  ...
  then
    ...
    -- Fix-Abo oder Cold-Start: einfaches Top-N nach preliminary_final
...
comment on function public.rank_coffees_for_customer(uuid, text, integer, boolean, uuid, date, text) is
  'Playbook 9.8: volles Ranking mit Hartfilter-Cascade, 7-Dim-Scoring, Vector-Sim, MMR (Discovery), explain_coffee_match. #variable_conflict use_column behebt OUT-vs-Spalten-Ambiguitaet.';
