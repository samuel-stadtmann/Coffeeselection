-- =============================================================================
-- Migration 024 — Fix rank_coffees_for_customer: DROP IF EXISTS fuer Temp-Tabellen
-- =============================================================================
-- Bug: Bei mehrfachem Aufruf in derselben Session existieren tmp_eligible etc.
-- noch vom vorigen Aufruf -> 42P07 "relation already exists".
-- Fix: DROP TABLE IF EXISTS am Anfang der Funktion.
-- =============================================================================
...
begin
  --------------------------------------------------------------------------
  -- 0) Temp-Tabellen aus vorherigen Aufrufen aufraumen
  --------------------------------------------------------------------------
  ...
  --------------------------------------------------------------------------
  -- 3) Hartfilter mit Fallback-Cascade
  --------------------------------------------------------------------------
  ...
  --------------------------------------------------------------------------
  -- 5) MMR fuer Discovery | Top-N fuer Fix
  --------------------------------------------------------------------------
  ...
  if p_save_snapshot and p_subscription_id is not null then
    ...
    values (
      ...
      (select jsonb_agg(jsonb_build_object(
          'rank', r_rank, 'coffee_id', r_coffee_id, 'final_score', r_final_score
        ) order by r_rank)
       from tmp_results),
      now()
    );
  ...
  --------------------------------------------------------------------------
  return query
    select r_rank, r_coffee_id, r_coffee_name, r_roaster_id,
           r_final_score, r_scoring_score, r_vector_similarity, r_reasons
    from tmp_results
    order by r_rank;
end;
$$;
