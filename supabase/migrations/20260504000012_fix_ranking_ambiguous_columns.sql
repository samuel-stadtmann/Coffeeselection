-- =============================================================================
-- Migration 022 — Fix rank_coffees_for_customer: ambiguous column refs
-- =============================================================================
-- Bug: UPDATE tmp_scored SET preliminary_final = ... * scoring_score ...
-- Fehler 42702: "scoring_score" und "vector_similarity" sind gleichzeitig
-- OUT-Variablen (RETURNS TABLE) und Spaltennamen in tmp_scored.
-- Fix: Tabellen-Alias 'ts' in der UPDATE-Anweisung verwenden.
-- =============================================================================
...
begin
  ...
  --------------------------------------------------------------------------
  -- 2) Hartfilter mit Fallback-Cascade
  --------------------------------------------------------------------------
  ...
  create temporary table tmp_scored (
    ...
    t_scoring_score    numeric,   -- Prafix 't_' vermeidet Kollision mit OUT-Vars
  ...
  -- preliminary_final ohne Alias-Konflikt
  ...
  --------------------------------------------------------------------------
  -- 5) MMR fuer Discovery | einfaches Top-N fuer Fix
  --------------------------------------------------------------------------
  ...
  then
    ...
    create temporary table tmp_pool on commit drop as
      select coffee_id, roaster_id, coffee_name, coffee_embedding,
             t_scoring_score, t_vector_sim, preliminary_final
      from tmp_scored
    ...
    while v_rank_counter < p_limit and exists (select 1 from tmp_pool) loop
      ...
      for r in select * from tmp_pool loop
        -- Relevanz-Teil: lambda * preliminary_final
        v_curr_mmr := v_mmr_lambda * r.preliminary_final;

        -- Diversitaets-Abzug: (1-lambda) * max_cosine_sim zu bereits ausgewaehlten
        if array_length(v_selected_embs, 1) is not null and r.coffee_embedding is not null then
          ...
          for i in 1..array_length(v_selected_embs, 1) loop
            v_max_sim := greatest(v_max_sim,
              1 - (r.coffee_embedding <=> v_selected_embs[i])
            );
          end loop;
      ...
      -- Diversity-Bonus: 1 - max_sim zu bereits Ausgewaehlten, skaliert auf 0-100
      ...
      begin
        if array_length(v_selected_embs, 1) > 1 and v_pick.coffee_embedding is not null then
          ...
          for i in 1..array_length(v_selected_embs, 1) - 1 loop
            v_max_sim := greatest(v_max_sim,
              1 - (v_pick.coffee_embedding <=> v_selected_embs[i])
            );
          end loop;
      ...
      delete from tmp_pool where coffee_id = v_pick.coffee_id;
    end loop;

  else
    -- Fix-Abo oder keine Embeddings: einfaches Top-N
  ...
  if p_save_snapshot and p_subscription_id is not null then
    ...
    values (
      ...
      (select jsonb_agg(jsonb_build_object(
          'rank', r_rank, 'coffee_id', r_coffee_id,
          'final_score', r_final_score
        ) order by r_rank)
       from tmp_results),
      now()
    );
